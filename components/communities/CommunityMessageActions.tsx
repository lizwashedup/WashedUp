 ...
 * Keeping the controls visible makes those actions available without requiring
 * people to discover a different long-press gesture in every conversation.
 */
export function CommunityMessageActions({ message, onError }: Props) {
  ...
  const react = async (emoji: string) => {
    ...
    try {
      hapticLight();
      if (mine?.emoji === emoji) {
        await toggleBroadcastReaction(message.id, emoji, false);
      } else {
