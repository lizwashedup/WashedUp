...
- **"past the minimum" pill on first-join surfaces** (`FirstJoinPlanCard`, `Colors.pastMinimumGreen` #2E7D32 + `pastMinimumGreenTint`). The ONE permitted green in the app, ordered by the first-join spec (b4): a factual, non-tappable pill stating member_count >= min_invites. It is information ("this plan clears its bar"), not a success celebration, which is why it does not use gold: gold on these cards already means honest scarcity ("2 spots left", `spotsLeftGoldFill`), and one color carrying both meanings on one row would blur both. Never use this green anywhere else, never for text outside this pill, never for CTAs.
