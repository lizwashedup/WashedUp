...
- **Paid-ticket setup is one journey, not a set of screens.** Before any paid-ticket release, prove Community and Organization event creation, persisted event end time, named paid tier creation, on-sale readiness, and the buyer checkout contract in one local gate. Ticket setup must never open from an existing draft until the complete event save finishes. Every paid-tier write rechecks the persisted event end time. Run `npm run qa:paid-ticket-flow` and the full `npm run qa:all` before release certification.
- **Free-first release routing is mandatory.** JavaScript-compatible changes use guarded OTA after explicit approval. Native runtime changes use a local signed build and upload after explicit approval. Active tooling must contain no cloud `eas build` or `eas submit` command; `npm run qa:no-cloud-build` enforces this. Local repair, commit, push, OTA, binary build, and store upload are separate states.
- **Every physical-device acceptance failure becomes a named regression contract before the next build.** Learned from Liz's Build 42 ticket-editor test on 2026-09-07: a long keyboard-heavy form must not ship inside a custom partial-height modal, a blocked save action must state the exact missing requirement instead of becoming a dead button, and a successful write must be re-read into the visible list before the UI reports success. Local release certification must cover these contracts. Physical-device behavior is only certified after the resulting TestFlight build is checked on-device.
- **When a "pick the right item from a list" bug is found and fixed in one place, grep the whole app for every other raw occurrence of the same anti-pattern before calling the bug class closed.** Found 2026-09-05: `app/(creator)/today.tsx` showed a dead months-old event as "your next event" because `events[0] ?? null` had no future/live filter. The correct, tested fix (`pickNextUpcomingEvent` in `lib/organizerHome.ts`) already existed in the codebase for a sibling screen, written by an earlier session that fixed it in exactly one place and never swept for the same raw pattern elsewhere. A grep the same night found a second live instance (`lib/communityPage.ts`'s events query powering the public community page) with the identical missing-date-filter bug, now also fixed (added `.gte('event_date', todayStr)`, matching the pattern already used correctly in `lib/communityChat.ts` and `lib/organizerProfile.ts`). Liz hit this live before any session caught it. Same root cause as the existing "map the whole app before fixing" rule below: this is that rule applied specifically to "a fix landed in one call site of a repeated pattern."
