...
--
-- BUG FIX, not a feature change. This file supersedes
-- supabase/migrations/20260817180000_community_co_creator_invites.sql, which
-- has NEVER successfully applied to any database. Confirmed live 2026-09-05:
--   * to_regclass('public.community_creator_invites') -> null (table absent)
--   * pg_proc has zero rows for create_/preview_/accept_/revoke_co_creator_invite
--   * supabase_migrations.schema_migrations has no row for version 20260817180000
-- Root cause: 20260817180000's create_co_creator_invite() declares
-- `p_role public.community_member_role DEFAULT 'admin'`. Postgres coerces a
-- typed parameter's DEFAULT literal against its declared enum type at
-- CREATE FUNCTION time, not at call time -- 'admin' is not a member of the
-- real, live community_member_role enum, so the CREATE OR REPLACE FUNCTION
-- statement itself raises 22P02 (invalid_text_representation) and aborts the
-- whole single-transaction migration. Nothing after it in that file ever ran.
-- Live enum check, same day: SELECT enumlabel FROM pg_enum WHERE enumtypid =
-- 'public.community_member_role'::regtype -> exactly leader, co_leader,
-- member. This matches (and is independently corroborated by)
-- 20260901000000_build35_community_role_reconciliation.sql's own header,
-- which documents that 20260821010000_community_role_tiers_enum.sql (the
-- migration that was meant to ALTER TYPE ... ADD VALUE the four S-03 labels
-- admin/events/member_care/finance) was moved to
-- docs/database/superseded-migrations/ by commit 4281ab8 and marked ARCHIVED
-- / NON-EXECUTABLE -- those four labels exist only in lib/creatorMode.ts's
-- CoCreatorRole type, never in the database.
--
-- THE FIX (one line): p_role now defaults to 'co_leader', the one tier that
-- is actually a real member of today's 3-label enum and already satisfies
-- this table's own CHECK constraint below. Nothing else in the original
-- design changes -- see the rest of this header (kept from the original
-- file) for the full feature rationale, still accurate.
--
-- The original 20260817180000 file is left in place, untouched, by this
-- session: it has never run anywhere so nothing depends on its current
-- bytes, but docs/database/migration-provenance.json (where a decision like
-- archiving it would normally be recorded, per the precedent set for the
-- S-03 enum migration above) is being edited by another in-flight session
-- tonight and is out of scope here. It is harmless to leave as-is: its
-- BEGIN/COMMIT wrapping means any future accidental apply attempt fails
-- atomically and rolls back cleanly, exactly as it does today. Josh should
-- decide whether to archive it (matching the S-03 precedent) or leave it;
-- this file is what should actually be reviewed and applied instead.
--
-- ============================================================================
...
-- co-creator at a chosen tier (p_role -- S-03's intent was
-- 20260821010000/20260821020000: admin/events/member_care/finance, never
-- leader or member, but that enum-widening migration was archived and never
-- applied -- see the DRAFT note at the very top of this file), who lands
-- with that role on community_members on acceptance. Originally this table
-- only ever granted 'co_leader'; the CHECK constraint below and every
-- hardcoded literal were widened for S-03 without otherwise touching this
-- file's structure. 'co_leader' remains a valid, permanent role (S-03's
-- Admin tier synonym, and the ONLY one of the intended tiers that is
-- actually usable against today's real enum); nothing here migrates old rows.
--
...
--     first written, so it needs no edit if a real, live-verified
--     ALTER TYPE ... ADD VALUE migration for those four labels is ever
--     actually applied (S-03's attempt at this,
--     20260821010000_community_role_tiers_enum.sql, was archived and never
--     applied -- see the DRAFT note at the top of this file) -- they would be
--     admitted automatically, no edit to this file required.
...
CREATE OR REPLACE FUNCTION public.create_co_creator_invite(
  ...
  -- FIX (this file): was DEFAULT 'admin', which is not a member of the real
  -- 3-label community_member_role enum and made this CREATE FUNCTION
  -- statement itself fail with 22P02 on any apply attempt -- see the DRAFT
  -- note at the top of this file. 'co_leader' is the one grantable tier that
  -- actually exists today and already satisfies the table's own
  -- CHECK (role NOT IN ('leader', 'member')) below.
  p_role           public.community_member_role DEFAULT 'co_leader'
)
