 ...
 *      TRUE → done + a durable doc-112 BUYER CONFIRMATION EMAIL job (Resend,
 *      one bounded provider call per queue attempt — see doc 112 + the
 *      2026-08-29 retry/observability hardening below): the address is read
 *      from session.customer_details.email in THIS payload and never
 *      persisted onto ticket_orders; a stable Resend Idempotency-Key makes
 *      provider retries safe; the SAME captured payload lives in a
 *      ticket_webhook_events row so the
...
// payload never had a buyer email or order id, so no retry, immediate or
...
service: any, obj: any) {
  try {
    ...
    if (!email || !orderId) {
      ...
      // provider retry and no durable retry row built from this same `obj`
    ...
    // Provider idempotency, not a speculative "sent" timestamp, owns the
    // send race. This avoids the crash window where the database said sent
    // before Resend was ever called. Replays use the same provider key and
    // receive the same email id without delivering a duplicate.
    const { data: order, error: orderErr } = await service.from('ticket_orders')
      .select('id, reference_code, qty, total_cents, event_id, status, confirmation_email_sent_at')
      .eq('id', orderId).maybeSingle();
    if (orderErr) return { sent: false, retryable: true, reason: `order read failed: ${orderErr.message}` };
    if (!order) return { sent: false, retryable: false, reason: 'order not found' };
    if (order.confirmation_email_sent_at) return { sent: true };
    if (order.status !== 'paid') return { sent: false, retryable: true, reason: `order is ${order.status}, waiting for settlement` };

    const { data: event, error: eventErr } = await service.from('explore_events').select('title, event_date, venue, confirmation_message').eq('id', order.event_id).maybeSingle();
    if (eventErr) return { sent: false, retryable: true, reason: `event read failed: ${eventErr.message}` };
    const { data: seats, error: seatsErr } = await service.from('ticket_order_positions').select('position_index, reference_code').eq('order_id', orderId).order('position_index', {
      ascending: true
    });
    if (seatsErr) return { sent: false, retryable: true, reason: `ticket references read failed: ${seatsErr.message}` };
    ...
    const refs: string[] = (seats ?? []).map((p: { reference_code: string })=>p.reference_code);
...
<div style="background:#FAF5EC;padding:32px 16px;font-family:-apple-system,'Segoe UI',sans-serif;color:#2C1810;">
  <div style="max-width:480px;margin:0 auto;background:#FFFFFF;border-radius:16px;padding:28px;">
    ...
    // Exactly one bounded provider call per durable queue attempt. The queue
    // itself owns retries and the stable Idempotency-Key makes them safe.
    let res = null;
    let lastStatus = null;
    ...
    for(let attempt = 0; attempt < CONFIRMATION_SEND_ATTEMPTS; attempt++){
      const r = await fetchWithTimeout('https://api.resend.com/emails', {
        ...
        headers: {
          ...
          'Content-Type': 'application/json',
          'Idempotency-Key': confirmationIdempotencyKey(orderId)
        },
        body: JSON.stringify({
    ...
    if (!emailId) return { sent: false, retryable: true, reason: 'provider success response missing email id' };
    const { error: stampErr } = await service.from('ticket_orders').update({
      confirmation_email_sent_at: new Date().toISOString(),
      confirmation_email_id: emailId
    }).eq('id', orderId);
    if (stampErr) return { sent: false, retryable: true, reason: `confirmation stamp failed: ${stampErr.message}` };
    return { sent: true };
  } catch (e) {
...
// A confirmation send is processed by the SAME drain as a durable row,
// never a second bespoke retry system.
...
service: any, obj: any, reason: string) {
  const email = obj?.customer_details?.email ?? null;
  const orderId = obj?.metadata?.order_id ?? null;
  if (!email || !orderId) return { ok: false, error: 'missing buyer email or order id' };
  const { error } = await service.from('ticket_webhook_events').upsert({
    // Stable key means a webhook replay cannot create a second delivery row.
    stripe_event_id: `confirmation_delivery_${orderId}`,
...
async function cancelConfirmationDelivery(// deno-lint-ignore no-explicit-any
service: any, obj: any, reason: string) {
  const orderId = obj?.metadata?.order_id ?? null;
  if (!orderId) return { ok: true };
  const { data, error } = await service.from('ticket_webhook_events').update({
    processed_at: new Date().toISOString(),
    error: `confirmation not sent: ${reason}`
  }).eq('stripe_event_id', `confirmation_delivery_${orderId}`).is('processed_at', null).select('id');
  if (error) return { ok: false, error: error.message };
  // Zero rows is safe: either no delivery was queued or another run already
  // finished/cancelled it.
...
Deno.serve(async (req)=>{
  ...
  // Keep slow provider work from monopolizing the settlement queue while
  // reserving capacity for older retries. Fresh work gets most slots, but a
  // continuous stream of new rows can never starve retry/poison progression.
  const CONFIRMATION_BATCH_SIZE = 2;
  const EVENT_RETRY_SLOTS = 2;
  const eventSlots = BATCH_SIZE - CONFIRMATION_BATCH_SIZE;
  const selection = 'id, stripe_event_id, type, payload, attempts';
  const { data: eventFresh, error: eventFreshErr } = await service.from('ticket_webhook_events').select(selection).is('processed_at', null).neq('type', 'confirmation_email_retry').eq('attempts', 0).order('id', {
    ascending: true
  }).limit(eventSlots);
  const { data: eventRetries, error: eventRetryErr } = await service.from('ticket_webhook_events').select(selection).is('processed_at', null).neq('type', 'confirmation_email_retry').gt('attempts', 0).order('id', {
    ascending: true
  }).limit(EVENT_RETRY_SLOTS);
  const { data: confirmationFresh, error: confirmationFreshErr } = await service.from('ticket_webhook_events').select(selection).is('processed_at', null).eq('type', 'confirmation_email_retry').eq('attempts', 0).order('id', {
    ascending: true
  }).limit(CONFIRMATION_BATCH_SIZE);
  const { data: confirmationRetries, error: confirmationRetryErr } = await service.from('ticket_webhook_events').select(selection).is('processed_at', null).eq('type', 'confirmation_email_retry').gt('attempts', 0).order('id', {
    ascending: true
  }).limit(1);
  if (eventFreshErr || eventRetryErr || confirmationFreshErr || confirmationRetryErr) return json(500, {
    error: 'inbox read failed'
  });
  const eventRetryRows = eventRetries ?? [];
  const confirmationRetryRows = confirmationRetries ?? [];
  const rows = [
    ...(eventFresh ?? []).slice(0, eventSlots - eventRetryRows.length),
    ...eventRetryRows,
    ...(confirmationFresh ?? []).slice(0, CONFIRMATION_BATCH_SIZE - confirmationRetryRows.length),
    ...confirmationRetryRows,
  ];
  let drained = 0;
  let failed = 0;
  let skipped = 0;
  for (const row of rows ?? []){
    ...
    const stampProcessed = async (errNote: string | null)=>{
      await service.from('ticket_webhook_events').update({
        processed_at: new Date().toISOString(),
        error: errNote
      }).eq('id', row.id);
    };
    const recordRetryable = async (errNote: string)=>{
    ...
    const stampFailed = async (errNote: string)=>{
      await service.from('ticket_webhook_events').update({
        processed_at: new Date().toISOString(),
        error: errNote,
        alert_state: 'open'
    ...
    try {
      ...
      if (row.type === 'account.updated') {
        ...
        if (!holdId) {
          ...
          const hasDeliveryPayload = typeof obj?.customer_details?.email === 'string'
            && obj.customer_details.email.trim().length > 0
            && typeof meta.order_id === 'string'
            && meta.order_id.trim().length > 0;
          if (hasDeliveryPayload) {
            // The outbox row must exist before settlement commits. A crash
            // after this point leaves a safe pending delivery that refuses to
            // send until ticket_orders.status becomes paid.
            const queued = await enqueueConfirmationRetry(service, obj, 'new paid order');
            if (!queued.ok) {
              await recordRetryable(`confirmation email queue failed before settlement: ${queued.error}`);
              continue;
            }
          }
          ...
          if (rpcErr) {
            ...
            if (isBusinessRaise) {
              const canceled = await cancelConfirmationDelivery(service, obj, `settle raised: ${rpcErr.message}`);
              if (!canceled.ok) {
                await recordRetryable(`could not cancel confirmation delivery: ${canceled.error}`);
                continue;
              }
            ...
            if (hasDeliveryPayload) {
              ...
              await stampFailed('confirmation email payload missing buyer email or order id — tickets issued, buyer never notified');
            }
          } else {
            // A crash after a successful settlement can replay this Stripe
            // row after the hold is already consumed. Check the durable order
            // truth before canceling its valid confirmation delivery.
            const orderId = typeof meta.order_id === 'string' ? meta.order_id : null;
            const { data: settledOrder, error: settledOrderErr } = orderId
              ? await service.from('ticket_orders').select('status').eq('id', orderId).maybeSingle()
              : { data: null, error: null };
            if (settledOrderErr) {
              await recordRetryable(`settlement replay status read failed: ${settledOrderErr.message}`);
              continue;
            }
            if (settlementFalseDisposition(settledOrder?.status ?? null) === 'already_paid') {
              await stampProcessed('settlement replay found paid order; confirmation delivery preserved');
              continue;
            }
            // Genuinely unsettled: money was captured, quota is gone, and a
            // human owes this buyer a refund.
            const canceled = await cancelConfirmationDelivery(service, obj, 'settlement refused, refund owed');
            if (!canceled.ok) {
              await recordRetryable(`could not cancel confirmation delivery: ${canceled.error}`);
              continue;
            }
        ...
        // runs the identical send path with the same stable provider
        // idempotency key and bounded timeout.
